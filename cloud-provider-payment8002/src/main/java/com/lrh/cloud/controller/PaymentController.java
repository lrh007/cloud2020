...
public class PaymentController {
    ...
    public CommonResult<Payment> addPayment(@RequestBody Payment payment) {
        ...
        if(i==0){
            return new CommonResult<>(444,"8002-插入数据库失败",null);
        }else{
            return new CommonResult<>(200,"8002-插入数据库成功",payment);
        }
    }
    ...
    public CommonResult<Payment> getPayment(@PathVariable Integer id){
        ...
        if(payment == null){
            return new CommonResult<>(444,"8002-没有对应记录，查询ID："+id,null);
        }else{
            return new CommonResult<>(200,"8002-查询成功",payment);
        }
    }
